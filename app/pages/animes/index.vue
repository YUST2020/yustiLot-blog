...
import { Calendar, Star, SortAsc, SortDesc, LayoutGrid, List, Loader2 } from 'lucide-vue-next'
...
const { data, pending } = await useFetch('/api/animes', {
  query: {
    page,
    pageSize,
    sortBy,
  ...
  watch: [page, viewMode, order]
})

const allAnimes = ref<any[]>(data.value?.items || [])

// 监听数据变化，累加到列表
watch(data, (newData) => {
  if (newData?.items) {
    if (page.value === 1) {
      allAnimes.value = newData.items
    } else {
      // 避免重复添加
      const existingIds = new Set(allAnimes.value.map(a => a.id))
      const newItems = newData.items.filter((a: any) => !existingIds.has(a.id))
      allAnimes.value.push(...newItems)
    }
    hasMore.value = allAnimes.value.length < (newData.total || 0)
  }
})
...
watch([viewMode, order], () => {
  page.value = 1
  // 不立即清空，让 useFetch 的 pending 触发骨架屏，避免闪烁到“暂无记录”
})
    ...
    <div class="min-h-[600px] relative pb-20">
      <Transition name="page-fade" mode="out-in">
        ...
        <div v-if="pending && page === 1" key="skeleton" class="space-y-16 relative">
          <div class="absolute left-4 md:left-1/2 top-0 bottom-0 w-0.5 bg-white/5 -translate-x-1/2 hidden md:block"></div>
          
          <div v-for="i in 2" :key="i" class="relative">
            <!-- Skeleton Header -->
            <div class="flex items-center justify-center mb-10 relative z-10">
              <div class="w-48 h-12 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 animate-pulse"></div>
            </div>
            
            <!-- Skeleton Grid -->
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 px-4 md:px-0">
              <div v-for="j in 5" :key="j" class="aspect-[3/4] rounded-xl bg-white/5 animate-pulse border border-white/10"></div>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="groupedItems.length" :key="viewMode + order" class="space-y-16 relative">
          ...
          <div class="absolute left-4 md:left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-white/20 via-white/10 to-transparent -translate-x-1/2 hidden md:block"></div>

          <TransitionGroup name="list" appear>
            <div v-for="(group, groupIndex) in groupedItems" :key="group.label" class="relative group/section">
              <!-- Group Header (Timeline Node) -->
              <div class="flex items-center justify-center mb-10 relative z-10">
                <!-- Node Link Line Connector (Vertical line behind node) -->
                <div class="hidden md:block absolute left-1/2 -translate-x-1/2 w-4 h-4 rounded-full border-2 border-white/50 bg-primary/20 backdrop-blur-sm z-0"></div>
                
                <!-- Glass Header -->
                <div class="relative bg-white/5 backdrop-blur-xl px-8 py-2.5 rounded-2xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.2)] transition-all duration-500 group-hover/section:bg-white/10 group-hover/section:-translate-y-1 group-hover/section:border-white/20 group-hover/section:shadow-primary/10">
                  <h2 class="text-xl md:text-2xl font-bold text-white tracking-tight flex items-center gap-4">
                    <div class="w-2 h-2 rounded-full bg-primary shadow-[0_0_8px_theme(colors.primary.DEFAULT)]"></div>
                    {{ group.label }}
                    <span class="text-xs bg-white/10 px-2.5 py-1 rounded-lg text-white/60 font-medium border border-white/5">
                      {{ group.items.length }}
                    </span>
                  </h2>
                </div>
              ...
              >
                ...
                >
                  ...
                  <!-- Overlay Base (Dynamic Gradient) -->
                  <div class="absolute inset-0 bg-gradient-to-t from-black/95 via-black/40 to-transparent opacity-90 group-hover:opacity-0 transition-opacity duration-500"></div>
                  
                  <!-- Hover Glass Overlay (Refined) -->
                  <div class="absolute inset-0 bg-black/60 backdrop-blur-[8px] opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col p-4 border border-white/10 rounded-xl">
                  </div>

                  <!-- Content Layout -->
                  <div class="absolute inset-0 p-4 flex flex-col z-10">
                    <!-- Main Content Wrapper: Translates from bottom to top on hover -->
                    <!-- Initial state: translate down so title/rating/preview are at the bottom -->
                    <div class="flex flex-col h-full transition-transform duration-500 ease-out group-hover:translate-y-0 translate-y-[calc(100%-100px)]">
                      
                      <!-- Title & Stars Section -->
                      <div class="space-y-1.5 flex-shrink-0">
                        <Badge variant="secondary" class="bg-white/20 backdrop-blur-md border-white/30 text-white text-[10px] py-0">
                        ...
                        <div class="flex items-center gap-1">
                          <div class="flex items-center gap-0.5">
                            <template v-for="star in renderStars(anime.rating)" :key="star.key">
                              ...
                              <Star 
                                v-else 
                                class="w-3 h-3 text-yellow-400/30" 
                              />
                            </template>
                          </div>
                      ...
                      <div class="relative mt-3 flex-1">
                        <!-- 1. Single Line Preview (Visible initially at the bottom, fades on hover) -->
                        <div class="transition-all duration-500 group-hover:opacity-0 group-hover:invisible group-hover:translate-y-2">
                          <div v-if="anime.review" class="pt-1 border-t border-white/10">
                            <p class="text-[11px] text-white/50 italic truncate leading-none py-1">
                              {{ anime.review }}
                            </p>
                          </div>
                        </div>

                        <!-- 2. Full Review (Visible only when hovered at the top) -->
                        <div class="absolute inset-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-500 delay-150 translate-y-4 group-hover:translate-y-0">
                          <p v-if="anime.review" class="text-[11px] text-white/90 italic leading-relaxed line-clamp-[12]">
                            "{{ anime.review }}"
                          </p>
                        </div>
                      </div>
