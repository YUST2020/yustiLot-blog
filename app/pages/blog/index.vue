...
<template>
  <div class="container relative z-10 px-4 mx-auto pt-24 pb-16">
    ...
    <div class="bg-background/60 backdrop-blur-md border border-white/20 shadow-2xl rounded-3xl p-8 md:p-12 min-h-[80vh] flex flex-col">
      ...
      <div class="mb-8 text-center space-y-4" v-motion-slide-visible-once-bottom>
      ...
      <!-- Tag Filter -->
      <div class="mb-10 flex flex-wrap justify-center gap-3" v-motion-fade-visible>
        <button 
          @click="selectedTag = null"
          class="px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-300 border"
          :class="!selectedTag ? 'bg-white text-primary border-white shadow-md scale-105' : 'bg-white/10 text-white border-white/20 hover:bg-white/20'"
        >
          全部
        </button>
        <button 
          ...
          class="px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-300 border"
          :class="selectedTag === tag ? 'bg-white text-primary border-white shadow-md scale-105' : 'bg-white/10 text-white border-white/20 hover:bg-white/20'"
        >
          {{ tag }}
        </button>
      </div>
        ...
        >
          ...
          >
            ...
            <div class="aspect-[16/10] overflow-hidden relative">
               ...
               />
               <div v-else class="w-full h-full flex items-center justify-center bg-gradient-to-br from-secondary/50 to-secondary text-muted-foreground">
                 <span class="text-5xl opacity-20">📝</span>
               </div>
               
               <!-- Date Badge -->
               <div class="absolute top-4 left-4 bg-background/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-medium shadow-sm">
            ...
            <div class="flex-1 p-6 flex flex-col space-y-3">
              <div v-if="post.tags" class="flex flex-wrap gap-2">
                 <span v-for="tag in JSON.parse(post.tags).slice(0, 2)" :key="tag" class="text-xs font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-md">
                   #{{ tag }}
                 </span>
              </div>
              ...
              <div class="pt-4 mt-auto flex items-center text-sm font-medium text-primary opacity-0 -translate-x-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0">
      ...
      <!-- Empty State -->
      <div v-if="filteredPosts && filteredPosts.length === 0" class="flex flex-col items-center justify-center py-20 text-white/80">
...
.list-leave-active {
  position: absolute; 
  width: 100%; /* Need specific width for absolute items to not collapse */
  max-width: calc(100% / 3 - 2rem); /* Adjust based on grid columns */
}
