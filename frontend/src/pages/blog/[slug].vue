...
const loadPost = async () => {
  try {
    post.value = await fetchPostBySlug(route.params.slug as string);
    if (post.value?.title) document.title = post.value.title;
  } catch {
    post.value = null;
  }
};
...
<template>
    <ParticleBackground variant="blue">
      <div v-if="post" class="min-h-[80vh] flex flex-col items-center">
        <!-- Main Content Card -->
        <div
          class="w-full max-w-7xl bg-background/60 backdrop-blur-md border border-white/20 shadow-2xl rounded-3xl p-6 md:p-12 relative mx-4"
        >
          <!-- Breadcrumb and Back -->
          <div class="mb-8">
            <RouterLink
              to="/blog"
              class="inline-flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors text-sm font-medium group"
            >
          ...
          <header class="mb-10 space-y-6">
            <div v-if="post.tags" class="flex flex-wrap gap-2">
              <span
                v-for="tag in JSON.parse(post.tags)"
                :key="tag"
                class="inline-flex items-center gap-1 bg-primary/10 text-primary px-3 py-1 rounded-full text-sm font-medium cursor-default"
              >
                <Tag class="w-3 h-3" />
                {{ tag }}
              </span>
            ...
            <div
              class="flex flex-wrap items-center gap-6 text-muted-foreground text-sm font-medium border-b pb-8 border-border/50"
            >
          ...
          <div
            ...
            class="mb-12 rounded-2xl overflow-hidden shadow-lg border border-border/50"
          >
          ...
          <!-- Share / Footer -->
          <div
            class="mt-16 pt-8 border-t border-border/50 flex flex-col md:flex-row items-center justify-between gap-4"
          >
            <div class="text-sm text-muted-foreground">感谢阅读，希望这篇文章对你有所启发。</div>
